...
case "$cmd" in
  ...
  tht|pub-tht|host-add|host-rm|trust-add|trust-rm|forge-fusion|forge-division|forge-issuance|forge-transfer|clist|cget|vote|join|actualize|leave|set-voting|vote-proposed|vote-proposed|vote-current)
    if [ -z $user ]; then
